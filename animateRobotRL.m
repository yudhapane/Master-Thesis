function animateRobotRL(t, x)
    ...
    textHandle = text(-0.005, 0.041, 'Iteration: ');
    % animation
    for i = 1: N
        if mod(i,50) == 0
            set(textHandle, 'String', ['Iteration: ' int2str(i/502)]);
            %textHandle = text(0, 0.03, ['Iteration: ' int2str(i/502)]);
        end
        % get state
        q = x(i);
        q= -q;
